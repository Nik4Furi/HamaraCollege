...
<div id="sign col">
    ...
    if (isset($_POST['update'])) {
        ...
        $sql ="update addstudent set name='$name', semester = '$semester', branch = '$branch',email = '$email',clgid = '$clgid_new',password= '$password' where clgid = '$clgid' ";
        $result = mysqli_query($conn,$sql);
        if ($result) {
            echo "<script>alert('Your students details updated')</script>"; ?>
    <meta http-equiv="refresh" content="0; url = http://localhost/HamaraCollege/admin/admin.php" />
    <?php
        } 
        else{
            echo "<script>alert('Your students not inserted details updated')</script>";
        }
    }
    ?>
</div>
