...
$title = "Admin Dashboard add new Parent";
...
<div id="sign col">
    ...
    // echo "Can you inserted the db<br>";
     ...
     if (isset($_POST['add'])) {
        ...
        $sql = "insert into addparent(parent_name, parent_children_semester,  parent_children_branch, parent_email, parent_clgid,parent_password) values('$name','$semester','$branch','$email','$clgid','$password')";
        $result = mysqli_query($conn,$sql);
        // echo var_dump($result);
        if ($result) { 
            echo "<script>alert('You added new parents details')</script>"; ?>
            <meta http-equiv="refresh" content="0; url = http://localhost/HamaraCollege/admin/admin.php" />
            <?php
                } 
                else{
                    echo "<script>alert('You did not insert details of parents')</script>";
                }
    }
    ?>
