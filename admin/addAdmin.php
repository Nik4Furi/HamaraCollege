...
<div id="sign">
    ...
    // echo "Can you inserted the db<br>";
     ...
     if (isset($_POST['add'])) {
        ...
        $sql = "insert into addadmin(name, semester,  branch, email, clgid,password) values('$name','$semester','$branch','$email','$clgid','$password')";
        $result = mysqli_query($conn,$sql);
        // echo var_dump($result);
        if ($result) {
            echo "<script>alert('You added new admin details')</script>"; ?>
            <meta http-equiv="refresh" content="0; url = http://localhost/HamaraCollege/admin/admin.php" />
            <?php
                } 
                else{
                    echo "<script>alert('You did not insert details of admin')</script>";
                }
    }
    ?>
