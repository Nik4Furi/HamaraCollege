...
<div id="sign col">
    ...
    if (isset($_POST['update'])) {
        ...
        $sql ="update addteacher set name='$name', semester = '$semester', branch = '$branch',email = '$email',clgid = '$clgid_new',password= '$password' where clgid = '$clgid' ";
        $result = mysqli_query($conn,$sql);
        if ($result) {
            echo "<script>alert('You updated the  details of teacher')</script>"; ?>
    <meta http-equiv="refresh" content="0; url = http://localhost/HamaraCollege/admin/admin.php" />
    <?php
        } 
        else{
            echo "<script>alert('Your teachers details not inserted')</script>";
        }
    }
    ?>
</div>
